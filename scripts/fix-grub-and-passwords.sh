...
step "Installing GRUB EFI binary"
chroot "$MNT" grub-install \
    --target=x86_64-efi \
    --efi-directory=/boot/efi \
    --bootloader-id=OSI \
    --no-nvram --removable \
    --modules="part_gpt part_msdos fat ext2 linux normal" \
    --recheck

# Write the full boot config directly on the EFI partition.
# No configfile redirect, no search command — hardcoded (hd0,gpt2) which is
# always correct for a single-disk VM with GPT.
mkdir -p "$MNT/boot/efi/EFI/BOOT"
cat > "$MNT/boot/efi/EFI/BOOT/grub.cfg" << GCFG
insmod part_gpt
insmod ext2
set root=(hd0,gpt2)
set timeout=0
...
echo "    EFI grub.cfg written."
cat "$MNT/boot/efi/EFI/BOOT/grub.cfg"

step "Resetting passwords (bypassing PAM)"
# Remove stale lock files left by interrupted previous runs
